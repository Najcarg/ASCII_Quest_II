...
function sendCombatPotionJson(array $payload, int $status): never
{
...
if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
    ...
    sendCombatPotionJson([
        'success' => false,
        'message' => 'Invalid combat Potion request.',
    ], 405);
}
...
if (
    ...
    sendCombatPotionJson([
        'success' => false,
        'message' => 'Invalid combat Potion intent.',
    ], 422);
}
...
try {
    ...
    $state = CombatBootstrap::service(getDb())->usePotion(
        (int) $_SESSION['user_id'],
        (int) $_SESSION['character_id'],
        $requestToken,
    );
    sendCombatPotionJson($state, 200);
    ...
    error_log('Combat Potion domain failure: ' . $exception->getMessage());
    sendCombatPotionJson([
        'success' => false,
        'message' => 'Combat Potion is unavailable.',
    ], 422);
} catch (Throwable $exception) {
    error_log('Combat Potion failed: ' . $exception->getMessage());
    sendCombatPotionJson([
        'success' => false,
        'message' => 'Unable to process combat Potion. Please try again.',
    ], 500);
}
